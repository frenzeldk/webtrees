...
class FunctionsPrintFacts
{
    ...
    public static function printSourceStructure(Tree $tree, array $textSOUR): string
    {
        ...
        if ($textSOUR['PAGE'] !== '') {
            $html .= Factory::gedcomElement()->make('INDI:SOUR:PAGE')->labelValue($textSOUR['PAGE'], $tree);
        }

        if ($textSOUR['EVEN'] !== '') {
            $html .= Factory::gedcomElement()->make('INDI:SOUR:EVEN')->labelValue($textSOUR['EVEN'], $tree);

            if ($textSOUR['ROLE']) {
                $html .= Factory::gedcomElement()->make('INDI:SOUR:EVEN:ROLE')->labelValue($textSOUR['ROLE'], $tree);
            }
        }

        if ($textSOUR['DATE'] !== '') {
            $html .= Factory::gedcomElement()->make('INDI:SOUR:DATA:DATE')->labelValue($textSOUR['DATE'], $tree);
        }

        foreach ($textSOUR['TEXT'] as $text) {
            $html .= Factory::gedcomElement()->make('INDI:SOUR:DATA:TEXT')->labelValue($text, $tree);
        }

        if ($textSOUR['QUAY'] !== '') {
            $html .= Factory::gedcomElement()->make('INDI:SOUR:QUAY')->labelValue($textSOUR['QUAY'], $tree);
        }
